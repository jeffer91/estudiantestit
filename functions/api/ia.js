...
const HOSTS = new Set([
  ...
  'integrate.api.nvidia.com','models.github.ai','openrouter.ai',
  'router.huggingface.co','api.cloudflare.com'
]);
...
function endpoint(raw){const type=text(raw.tipo).toLowerCase();if(type==='gemini'||providerId(raw.id||raw._docId)==='gemini')return 'https://generativelanguage.googleapis.com/v1beta/models/'+encodeURIComponent(text(raw.modelo||raw.model||'gemini-2.0-flash'))+':generateContent';const value=text(raw.endpoint||raw.url);if(!value)throw new Error('El proveedor no tiene endpoint configurado.');const url=new URL(value);if(url.protocol!=='https:'||!HOSTS.has(url.hostname))throw new Error('El endpoint del proveedor no está permitido.');return url.toString();}
async function fetchTimed(url,options,ms){const c=new AbortController();const timer=setTimeout(()=>c.abort(),Math.min(60000,Math.max(5000,number(ms,45000))));try{return await fetch(url,{...options,signal:c.signal});}finally{clearTimeout(timer);}}
function extractText(data,type){if(type==='gemini')return text(data&&data.candidates&&data.candidates[0]&&data.candidates[0].content&&data.candidates[0].content.parts&&data.candidates[0].content.parts[0]&&data.candidates[0].content.parts[0].text);return text(data&&data.choices&&data.choices[0]&&data.choices[0].message&&data.choices[0].message.content||data&&data.result&&data.result.response);}
async function generate(raw,prompt,options){const key=secret(raw);if(!key)throw new Error('El proveedor no tiene clave privada configurada.');const type=text(raw.tipo).toLowerCase()==='gemini'||providerId(raw.id||raw._docId)==='gemini'?'gemini':'openai-compatible';let url=endpoint(raw);let body;let auth={};if(type==='gemini'){url+=(url.includes('?')?'&':'?')+'key='+encodeURIComponent(key);body={contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:number(options.temperatura,raw.temperatura||0.3),maxOutputTokens:number(options.maxTokens,raw.maxTokens||3000)}};}else{auth.Authorization='Bearer '+key;body={model:text(raw.modelo||raw.model),messages:[{role:'user',content:prompt}],temperature:number(options.temperatura,raw.temperatura||0.3),max_tokens:number(options.maxTokens,raw.maxTokens||3000)};}const start=Date.now();const response=await fetchTimed(url,{method:'POST',headers:{'Content-Type':'application/json',...auth},body:JSON.stringify(body)},options.timeoutMs||raw.timeoutMs);const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(text(data.error&&data.error.message||data.message||data.error)||'El proveedor IA respondió con error.');const output=extractText(data,type);if(!output)throw new Error('El proveedor IA respondió sin texto utilizable.');return{text:output,latencyMs:Date.now()-start};}
