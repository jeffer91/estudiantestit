/* Consultas indexadas para Coordinadores y Administrador.
 * Evita recorrer completa la colección envios en cada actualización.
 * - Coordinadores: consulta únicamente las carreras asignadas.
 * - Administrador/pendientes: cuando pide PENDIENTE_REVISION consulta primero
 *   los estados pendientes y luego filtra por carreras.
 * - Mantiene Artículo Académico + Trabajo de Titulación, todos los períodos y
 *   la deduplicación por cédula + período + tipo de trabajo.
 */
import {
  executeTitulosAction as executePrevious,
  publicTitleConfiguration
} from './titulos-firebase-v11.js';
import {
  normalizeCedula,
  periodSignature,
  queryIn,
...
const PENDIENTES_COORDINACION = new Set([
  'PENDIENTE_REVISION',
...
function normalizeStatus(value) {
  ...
  if (status === 'PENDIENTE_SYNC') return 'PENDIENTE_SYNC';
...
function statusMatches(row, requested) {
  const current = normalizeStatus(row && (row.estadoProceso || row.estado || row.estadoFinal));
  const wanted = normalizeStatus(requested);
...
async function indexedByCareers(careers, env) {
  const map = new Map();
  const names = [...new Set(careers.map(text).filter(Boolean))];
  const keys = [...new Set(names.map(careerKey).filter(Boolean))];
  const codeLike = names.filter((value) => /^[A-Za-z0-9_-]{2,40}$/.test(value) && !/\s/.test(value));

  const primary = await safeQuery('carreraNombre', names, env);
  addRows(map, primary);

  const legacy = await Promise.all([
    safeQuery('carrera', names, env),
    safeQuery('nombreCarrera', names, env),
    keys.length ? safeQuery('carreraClave', keys, env) : Promise.resolve([]),
    codeLike.length ? safeQuery('carreraCodigo', codeLike, env) : Promise.resolve([])
  ]);
  legacy.forEach((rows) => addRows(map, rows));
  return [...map.values()];
}
...
function periodKey(row) {
  row = row || {};
  const values = [
    row.periodoNombre, row.periodoLabel, row.periodoCanonicoLabel,
    row.periodoCanonicoId, periodFromId(row), row.periodoId,
    row.periodId, row.periodo
  ].map(text).filter(Boolean);
...
async function completeCoordinatorPopulation(payload, env) {
  ...
  const filtered = candidates.filter((row) => {
    ...
    if (!statusMatches(row, requestedState)) return false;
...
async function completeListResult(payload, env) {
  ...
  return {
    ...
    consultaIndexada: true,
    lecturaGlobalColeccion: false,
    fuenteEnvios: 'FIREBASE_TITULOS_ENVÍOS_INDEXADOS',
...
export async function executeTitulosAction(action, payload = {}, userRole = 'student', env) {
  ...
  const hasCareers = splitList(payload.carreras || payload.carrera || payload.nombreCarrera).length > 0;

  /* Importante: las acciones de listado con carreras se resuelven aquí y NO
     pasan por las capas anteriores. Así evitamos que una compatibilidad antigua
     haga una lectura completa de envios antes de devolver la respuesta. */
  if (LIST_ACTIONS.has(normalized) && hasCareers) {
    return completeListResult(payload, env);
  }
