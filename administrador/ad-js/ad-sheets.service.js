...
- Probar y ejecutar acciones reales del Apps Script.
- Exponer acciones seguras de mantenimiento de datos.
========================================================= */
(function(window){
  "use strict";
  ...
  function enviarGet(accion,payload){
    ...
    return leerConfiguracion().then(function(cfg){
      ...
      return fetch(construirUrlGet(cfg.endpoint,accion,payload||{},cfg),opciones)
        ...
        .then(
          function(data){if(timer)window.clearTimeout(timer);return{ok:true,status:200,data:data,configuracion:cfg};},
  ...
  function extraerLista(respuesta){
    ...
    var candidatos=[
      ...
      data.data&&data.data.pendientes,
      data.data&&data.data.casos
    ];
  ...
  function analizarGoogleSheets(payload){
    return enviarGet("ANALIZAR_GOOGLE_SHEETS",payload||{});
  }
  function corregirGoogleSheets(payload){
    return enviarPost("CORREGIR_GOOGLE_SHEETS",payload||{});
  }
  function listarHistorialReparaciones(limite){
    return enviarGet("LISTAR_HISTORIAL_REPARACIONES",{limite:Math.max(1,numero(limite,100))});
  }
  ...
  window.ADSheetsService={
    ...
    analizarGoogleSheets:analizarGoogleSheets,
    corregirGoogleSheets:corregirGoogleSheets,
    listarHistorialReparaciones:listarHistorialReparaciones,
