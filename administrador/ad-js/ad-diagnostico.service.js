...
- Entregar datos para dashboard, KPIs y logs administrativos.
Dependencias:
- ad-config.js
...
(function(window){
  ...
  function ocultarToken(valor){
    if (utils().ocultarToken) return utils().ocultarToken(valor);
    var limpio = texto(valor);
    return limpio ? limpio.slice(0, 8) + "******" : "";
  }

  function valorBooleano(valor){
    return valor === true || texto(valor).toLowerCase() === "true";
  }
  ...
  function extraerPeriodo(configApp){
    ...
    return {
      ...
      periodosActivos: Array.isArray(data.periodosActivos) ? data.periodosActivos : [],
      periodosActivosLabels: Array.isArray(data.periodosActivosLabels) ? data.periodosActivosLabels : []
    };
  }
  ...
  function revisarColeccion(nombre, limite, opciones){
    ...
    if (opts.orden) {
      tareaMuestra = firebaseService().listarColeccionOrdenada(nombre, opts.orden, opts.direccion || "desc", limite || 5)
        .catch(function(){
          return firebaseService().listarColeccion(nombre, limite || 5);
        });
    } else {
      tareaMuestra = firebaseService().listarColeccion(nombre, limite || 5);
    }

    return Promise.all([
      firebaseService().contarColeccion(nombre).catch(function(error){
        return { ok: false, total: 0, error: error.message || String(error), metodo: "error" };
      }),
      tareaMuestra.catch(function(error){
        return { ok: false, totalLeido: 0, datos: [], error: error.message || String(error) };
      })
    ]).then(function(partes){
      var conteo = partes[0] || {};
      var muestra = partes[1] || {};
      return {
        nombre: nombre,
        ok: Boolean(conteo.ok || muestra.ok),
        total: Number(conteo.total || 0),
        metodoConteo: conteo.metodo || "sin conteo",
        totalLeido: Number(muestra.totalLeido || 0),
        muestra: muestra.datos || [],
        error: conteo.error || muestra.error || ""
      };
  ...
  function obtenerLogsRecientes(limite){
    ...
    return firebaseService().listarColeccionOrdenada(colecciones.logs, "fecha", "desc", max)
      .catch(function(){
        return firebaseService().listarColeccionOrdenada(colecciones.logs, "fechaCliente", "desc", max);
      })
      .catch(function(){
        return firebaseService().listarColeccionOrdenada(colecciones.logs, "creadoEn", "desc", max);
      })
      .catch(function(){
        return firebaseService().listarColeccion(colecciones.logs, max);
      })
  ...
  function probarFirebase(){
    var colecciones = config().colecciones || {};
    var documentoConfig = config().documentos && config().documentos.appConfig;

    return firebaseService().inicializar()
      .then(function(){
        return firebaseService().leerDocumento(colecciones.titulosConfig, documentoConfig);
      })
      .then(function(configResultado){
        var configApp = configResultado.data || {};
        var tareas = [
        ...
        return Promise.all(tareas).then(function(resumen){
          return {
            ok: true,
            proyecto: config().firebaseConfig.projectId,
            configExiste: configResultado.existe,
            configApp: configApp,
            periodo: extraerPeriodo(configApp),
            sheets: extraerSheets(configApp),
            colecciones: resumen.slice(0, 5),
            logsRecientes: resumen[5] || []
          };
        });
      });
  }
  ...
  function probarSheets(){
    ...
    return firebaseService().leerDocumento(colecciones.titulosConfig, documentoConfig)
      .then(function(configResultado){
        ...
        var payload = Object.assign({}, config().sheets.pingPayload || {}, {
        ...
        if (!sheets.webAppUrl) {
          return {
            ok: false,
            sheets: sheets,
            mensaje: "No existe sheetsWebAppUrl en titulos_config/app."
          };
        }
        ...
        if (!sheets.activo) {
          return {
            ok: false,
            sheets: sheets,
            mensaje: "Google Sheets está configurado, pero sheetsActivo no está en true."
          };
        }

        return fetch(sheets.webAppUrl, {
          method: "POST",
          headers: {
          ...
          .then(function(respuesta){
            return respuesta.text().then(function(textoRespuesta){
              var json = null;
              try {
                json = JSON.parse(textoRespuesta);
              } catch (errorJson) {
                json = { raw: textoRespuesta };
              }
              return {
                ok: respuesta.ok,
                ...
                sheets: sheets,
                respuesta: json,
                mensaje: respuesta.ok ? "PING enviado a Google Sheets." : "Google Sheets respondió con error HTTP."
              };
            });
          })
          .catch(function(error){
  ...
  function resumenTextoFirebase(resultado){
    var lineas = [];
    var colecciones = resultado.colecciones || [];
    var i;
    ...
    for (i = 0; i < colecciones.length; i += 1) {
      lineas.push(
        "Colección " + colecciones[i].nombre + ": " +
        (colecciones[i].ok ? "ok | total " + colecciones[i].total + " | muestra " + colecciones[i].totalLeido : "error: " + colecciones[i].error)
      );
    }
  ...
  window.ADDiagnosticoService = {
    probarFirebase: probarFirebase,
    probarSheets: probarSheets,
    extraerPeriodo: extraerPeriodo,
    extraerSheets: extraerSheets,
    resumenTextoFirebase: resumenTextoFirebase,
    obtenerLogsRecientes: obtenerLogsRecientes
  };
})(window);
