/* Pantalla Estudiantes: controlador de ejecución */
(function(window,document){
  ...
  function etiquetaEstado(estado){
    if(estado==="NO_ENVIO")return{texto:"No envió",clase:"ad-badge-neutral"};
    if(estado==="DEVUELTO")return{texto:"Devuelto",clase:"ad-badge-danger"};
    if(estado==="APROBADO"||estado==="REEMPLAZADO")return{texto:"Aprobado",clase:"ad-badge-success"};
    return{texto:"Envió",clase:"ad-badge-info"};
  }
  ...
  function cargarEstudiantes(){
    ...
    return service().cargar(periodo).then(function(resultado){
      ...
      mostrarEstado(
        ...
        (resultado.sheetsDisponible?". Se cruzaron las revisiones disponibles en Google Sheets.":". Se usó la información disponible en Firebase; no se recibieron revisiones desde Google Sheets."),
        "success"
      );
  ...
  function iniciar(){
    var select,tabla,modal,vista;
    if(iniciado)return;
    iniciado=true;
    select=$("ad-estudiantes-periodo");
  ...
  window.ADEstudiantesRuntime={iniciar:iniciar,cargarPeriodos:cargarPeriodos,cargarEstudiantes:cargarEstudiantes,abrirModal:abrirModal};
})(window,document);
