...
(function(window,document){
  ...
  function refrescarTodo(){busy(true,'Cargando Firebase...');var partes=[];return Promise.allSettled([cargarPeriodos(),cargarCoordinadores(),cargarIA()]).then(function(iniciales){partes=iniciales;return cargarTitulos().catch(function(error){partes.push({status:'rejected',reason:error});});}).then(function(){var errores=partes.filter(function(p){return p.status==='rejected';}).map(function(p){return mensaje(p.reason);});estadoBox('ad-estado-general',errores.length?'Carga parcial:\n'+errores.join('\n'):'Datos cargados correctamente desde Firebase.',errores.length?'warning':'success');return diagnosticar();}).finally(function(){busy(false);});}
  ...
  function enlazar(){
    ...
    ['ad-filtro-titulo-carrera','ad-filtro-titulo-estado'].forEach(function(id){var el=$(id);if(el)el.addEventListener('change',renderTitulos);});
