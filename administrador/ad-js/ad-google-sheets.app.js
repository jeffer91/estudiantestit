...
(function(window,document){
  ...
  var state={
    ...
    periodoId:'',coordinadorEdicion:null,investigadorEdicion:null,proveedorEdicion:null,tituloActual:null,estadisticas:null
  };
  ...
  function etiquetaEstado(value){var e=texto(value).toUpperCase();if(e==='PENDIENTE_INVESTIGADOR')return'Pendiente de Investigación';if(e==='APROBADO_FINAL')return'Aprobado final';if(e==='APROBADO')return'Aprobado histórico';if(e==='REEMPLAZADO')return'Aprobado con corrección histórico';if(e==='DEVUELTO')return'Devuelto';return'Pendiente de Coordinación';}
  function claseEstado(value){var e=texto(value).toUpperCase();if(e==='APROBADO_FINAL'||e==='APROBADO'||e==='REEMPLAZADO')return'ad-badge-success';if(e==='DEVUELTO')return'ad-badge-warning';return'ad-badge-info';}
  ...
  function normalizarTitulo(t){
    ...
    return{id:texto(t.envioId||t.id||t._id||t.idRegistro),cedula:cedula(t.cedula||t.numeroIdentificacion),nombres:texto(t.estudiante||t.nombres||t.Nombres),carrera:texto(t.carrera||t.carreraNombre||t.NombreCarrera||t.nombreCarrera),periodo:periodLabel,periodoId:periodId,estado:texto(t.estadoProceso||t.estado||t.estadoFinal||'PENDIENTE_REVISION').toUpperCase(),fecha:texto(t.fechaEnvio||t['Fecha envío']||t.fechaServidor),titulo1:texto(t.titulo1||t['Título 1']),titulo2:texto(t.titulo2||t['Título 2']),titulo3:texto(t.titulo3||t['Título 3']),preferido:favorito,tituloPreferidoTexto:texto(t.tituloPreferidoTexto),tituloCoordinador:texto(t.tituloCoordinador||t.tituloValidadoCoordinador),tituloFinal:texto(t.tituloFinalInvestigacion||t.tituloFinal||t.tituloAprobado||t.tituloCorregido),coordinador:texto(t.coordinador||t.nombreCoordinador||t.ultimoCoordinador),observacion:texto(t.comentarioCoordinador||t.observacion||t.comentario),fechaResolucion:texto(t.fechaValidacionCoordinador||t.fechaResolucion||t.fechaRevision),investigador:'',investigadorId:'',investigacionResultado:texto(t.resultadoInvestigacion),investigacionFecha:texto(t.fechaResolucionInvestigacion),investigacionAntes:'',investigacionDespues:texto(t.tituloFinalInvestigacion||''),investigacionObservacion:texto(t.observacionInvestigacion||''),bloqueoInvestigacion:false,raw:t};
  }

  function aplicarResumenInvestigacion(){
    var resumen=state.investigacionResumen||{},mapa=resumen.ultimoPorEnvio||{},bloqueos={};
    (resumen.bloqueos||[]).forEach(function(b){bloqueos[texto(b.envioId)]=b;});
    state.titulos=state.titulos.map(function(t){var rev=mapa[t.id]||null,b=bloqueos[t.id]||null;if(!rev&&!b)return t;return Object.assign({},t,{investigador:rev&&rev.investigadorNombre||'',investigadorId:rev&&rev.investigadorId||'',investigacionResultado:rev&&rev.resultado||t.investigacionResultado,investigacionFecha:rev&&rev.fecha||t.investigacionFecha,investigacionAntes:rev&&rev.tituloAntes||'',investigacionDespues:rev&&rev.tituloDespues||t.investigacionDespues,investigacionObservacion:rev&&rev.observacion||t.investigacionObservacion,bloqueoInvestigacion:Boolean(b)});});
    llenarFiltrosResponsables();
  }

  function llenarFiltrosResponsables(){
    var c=$('ad-filtro-titulo-coordinador'),i=$('ad-filtro-titulo-investigador');
    if(c){var prev=texto(c.value);c.innerHTML='<option value="">Todos</option>'+state.coordinadores.map(function(x){return option(x.nombre,x.nombre,false);}).join('');if(prev)c.value=prev;}
    if(i){var prevI=texto(i.value);i.innerHTML='<option value="">Todos</option>'+state.investigadores.map(function(x){return option(x.nombre,x.nombre,false);}).join('');if(prevI)i.value=prevI;}
  }

  function filtrosTitulo(){return{periodo:texto($('ad-filtro-titulo-periodo')&&$('ad-filtro-titulo-periodo').value),carrera:texto($('ad-filtro-titulo-carrera')&&$('ad-filtro-titulo-carrera').value),estado:texto($('ad-filtro-titulo-estado')&&$('ad-filtro-titulo-estado').value),coordinador:texto($('ad-filtro-titulo-coordinador')&&$('ad-filtro-titulo-coordinador').value),investigador:texto($('ad-filtro-titulo-investigador')&&$('ad-filtro-titulo-investigador').value),buscar:normal($('ad-buscar-titulo')&&$('ad-buscar-titulo').value)};}
  function renderTitulos(){
    var f=filtrosTitulo();var lista=state.titulos.filter(function(t){if(f.periodo&&!mismoPeriodo(t.periodoId||t.periodo,f.periodo))return false;if(f.carrera&&normal(t.carrera)!==normal(f.carrera))return false;if(f.estado&&t.estado!==f.estado)return false;if(f.coordinador&&normal(t.coordinador)!==normal(f.coordinador))return false;if(f.investigador&&normal(t.investigador)!==normal(f.investigador))return false;if(f.buscar&&normal([t.cedula,t.nombres,t.carrera,t.periodo,t.estado,t.titulo1,t.titulo2,t.titulo3,t.tituloCoordinador,t.tituloFinal,t.coordinador,t.investigador].join(' ')).indexOf(f.buscar)<0)return false;return true;});
  ...
  function pintarInvestigacionDetalle(titulo){
    setTexto('ad-investigacion-estado',titulo.estado==='PENDIENTE_INVESTIGADOR'?'Pendiente de Investigación':(titulo.investigacionResultado||titulo.estado==='APROBADO_FINAL'?'Resuelta':'Sin revisión'));
    setTexto('ad-investigacion-investigador',titulo.investigador||'-');
    setTexto('ad-investigacion-fecha',fecha(titulo.investigacionFecha));
    setTexto('ad-investigacion-resultado',titulo.investigacionResultado||'-');
    setTexto('ad-investigacion-antes',titulo.investigacionAntes||titulo.tituloCoordinador||'-');
    setTexto('ad-investigacion-despues',titulo.investigacionDespues||titulo.tituloFinal||'-');
    setTexto('ad-investigacion-observacion',titulo.investigacionObservacion||'-');
    var liberar=$('ad-btn-liberar-investigacion');if(liberar)liberar.hidden=titulo.estado!=='PENDIENTE_INVESTIGADOR';
  }

  function renderLineaTiempo(historial){
    var eventos=historial&&historial.lineaTiempo||historial&&historial.eventosWorkflow||[];
    if(!eventos.length){setHtml('ad-detalle-linea-tiempo','<span class="ad-muted">No hay eventos adicionales registrados.</span>');return;}
    setHtml('ad-detalle-linea-tiempo',eventos.map(function(e){var cambio=e.tituloAntes&&e.tituloDespues&&normal(e.tituloAntes)!==normal(e.tituloDespues)?'<div><strong>Antes:</strong> '+esc(e.tituloAntes)+'</div><div><strong>Después:</strong> '+esc(e.tituloDespues)+'</div>':'';return '<div style="padding:10px 0;border-bottom:1px solid #e3e8ef"><strong>'+esc([e.rol,e.accion,e.resultado].filter(Boolean).join(' · '))+'</strong><br><small>'+esc(fecha(e.fecha))+'</small>'+cambio+(e.observacion?'<p>'+esc(e.observacion)+'</p>':'')+'</div>';}).join(''));
  }
  ...
  function abrirDetalle(titulo){
    ...
    var reviewed=['PENDIENTE_REVISION','PENDIENTE_COORDINADOR'].indexOf(titulo.estado)<0;setTexto('ad-detalle-estado',etiquetaEstado(titulo.estado));setTexto('ad-detalle-revisado',reviewed?'Sí':'No, todavía está pendiente');setTexto('ad-detalle-coordinador',titulo.coordinador||'-');setTexto('ad-detalle-fecha-resolucion',fecha(titulo.fechaResolucion));setTexto('ad-detalle-titulo-final',titulo.tituloCoordinador||titulo.tituloFinal||'-');setTexto('ad-detalle-observacion',titulo.observacion||'-');
    pintarInvestigacionDetalle(titulo);setHtml('ad-detalle-linea-tiempo','Cargando historial...');
    var reason=$('ad-modal-motivo-devolucion');if(reason)reason.value='';estadoBox('ad-modal-estado-titulo','Desde este modal puedes devolver, liberar una revisión de Investigación o eliminar el registro.','info');
    ...
    api().consultarHistorialTitulo({envioId:titulo.id,cedula:titulo.cedula,periodoId:titulo.periodoId,periodo:titulo.periodo}).then(renderLineaTiempo).catch(function(error){setTexto('ad-detalle-linea-tiempo','Historial no disponible: '+mensaje(error));});
  }
  ...
  function renderInvestigadores(){
    var filas=state.investigadores.map(function(i){return '<tr><td><strong>'+esc(i.nombre)+'</strong></td><td>'+esc(i.cedula)+'</td><td><span class="ad-badge '+(i.tienePin?'ad-badge-success':'ad-badge-warning')+'">'+(i.tienePin?'Registrado':'Pendiente de crear')+'</span></td><td><span class="ad-badge '+(i.activo?'ad-badge-success':'ad-badge-warning')+'">'+(i.activo?'Activo':'Inactivo')+'</span></td><td><button class="ad-btn ad-btn-secondary" type="button" data-action="editar-investigador" data-cedula="'+esc(i.cedula)+'">Editar</button> <button class="ad-btn ad-btn-secondary" type="button" data-action="reset-pin-investigador" data-cedula="'+esc(i.cedula)+'">Restablecer PIN</button></td></tr>';});
    setHtml('ad-tabla-investigadores',filas.length?filas.join(''):'<tr><td colspan="5" class="ad-empty">No hay investigadores registrados.</td></tr>');
    llenarFiltrosResponsables();
  }

  function editarInvestigador(c){var i=state.investigadores.find(function(x){return x.cedula===c;});if(!i)return;state.investigadorEdicion=i;$('ad-investigador-cedula').value=i.cedula;$('ad-investigador-nombre').value=i.nombre;$('ad-investigador-activo').checked=i.activo!==false;mostrarVista('ad-seccion-investigadores');}
  function guardarInvestigador(event){event.preventDefault();var c=cedula($('ad-investigador-cedula').value),n=texto($('ad-investigador-nombre').value),activo=$('ad-investigador-activo').checked;if(!/^\d{10}$/.test(c)||!n){estadoBox('ad-estado-investigador','Ingresa nombre y cédula válida de 10 dígitos.','danger');return;}busy(true,'Guardando investigador...');api().guardarInvestigador({cedula:c,nombre:n,activo:activo}).then(function(){estadoBox('ad-estado-investigador','Investigador guardado correctamente.','success');state.investigadorEdicion=null;$('ad-form-investigador').reset();$('ad-investigador-activo').checked=true;return cargarInvestigadores();}).catch(function(error){estadoBox('ad-estado-investigador',mensaje(error),'danger');}).finally(function(){busy(false);});}
  function resetPinInvestigador(c){if(!window.confirm('¿Restablecer el PIN de este investigador? Sus sesiones activas también se cerrarán.'))return;busy(true,'Restableciendo PIN...');api().resetPinInvestigador(c).then(function(r){estadoBox('ad-estado-investigador',r.mensaje||'PIN restablecido.','success');return cargarInvestigadores();}).catch(function(error){estadoBox('ad-estado-investigador',mensaje(error),'danger');}).finally(function(){busy(false);});}
  ...
  function periodoTitulosActual(forzado){var id=texto(forzado)||texto($('ad-filtro-titulo-periodo')&&$('ad-filtro-titulo-periodo').value)||state.periodoId;var item=buscarPeriodo(id)||state.principal||null;return{id:id||texto(item&&item.id),label:texto(item&&(item.label||item.periodoLabel))||id};}
  ...
  function cargarCoordinadores(){return api().listarCoordinadores().then(function(r){state.coordinadores=api().extraerCoordinadores(r);renderCoordinadores();llenarFiltrosResponsables();});}
  ...
  function refrescarTodo(){busy(true,'Cargando Firebase...');var partes=[];return Promise.allSettled([cargarPeriodos(),cargarCoordinadores(),cargarInvestigadores(),cargarResumenInvestigacion(),cargarIA()]).then(function(iniciales){partes=iniciales;return cargarTitulos().catch(function(error){partes.push({status:'rejected',reason:error});});}).then(function(){var errores=partes.filter(function(p){return p.status==='rejected';}).map(function(p){return mensaje(p.reason);});estadoBox('ad-estado-general',errores.length?'Carga parcial:\n'+errores.join('\n'):'Datos cargados correctamente desde Firebase.',errores.length?'warning':'success');return diagnosticar();}).finally(function(){busy(false);});}
  ...
  function migrarSecretosIA(){if(!window.confirm('¿Migrar las credenciales de IA que aún estén en Firestore a Firebase Secret Manager?'))return;busy(true,'Migrando secretos de IA...');api().migrarSecretosIA().then(function(r){estadoBox('ad-estado-ia',r.mensaje||'Migración de secretos completada.','success');return cargarIA();}).catch(function(error){estadoBox('ad-estado-ia',mensaje(error),'danger');}).finally(function(){busy(false);});}
  ...
  function enlazar(){
    ...
    var search=$('ad-buscar-titulo');if(search)search.addEventListener('input',renderTitulos);
    ['ad-filtro-titulo-carrera','ad-filtro-titulo-estado','ad-filtro-titulo-coordinador','ad-filtro-titulo-investigador'].forEach(function(id){var el=$(id);if(el)el.addEventListener('change',renderTitulos);});
