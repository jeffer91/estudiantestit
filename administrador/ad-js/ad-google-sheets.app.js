/* Administrador Google Sheets: Requisitos consulta, Títulos operación, Claves configuración. */
(function(window,document){
  ...
  function renderPeriodos(){
    var filas=state.periodos.map(function(p){return '<tr><td>'+esc(p.id||p.periodoId)+'</td><td>'+esc(p.label||p.periodoLabel||p.id)+'</td><td><span class="ad-badge ad-badge-success">Consulta</span></td><td>'+(p.principal?'<span class="ad-badge ad-badge-info">Principal</span>':'-')+'</td></tr>';});
    ...
    var selects=document.querySelectorAll('[data-periodo-select]');
    selects.forEach(function(el){el.innerHTML='<option value="">Todos / principal</option>'+state.periodos.map(function(p){return option(p.id||p.periodoId,p.label||p.periodoLabel||p.id,false);}).join('');});
  ...
  function renderCarreras(){
    var filas=state.carreras.map(function(c){var codigo=texto(c.CodigoCarrera||c.codigoCarrera||c.codigo||c.id);var nombre=texto(c.NombreCarrera||c.nombreCarrera||c.nombre||c.carrera);return '<tr><td>'+esc(codigo)+'</td><td>'+esc(nombre)+'</td><td><span class="ad-badge ad-badge-info">Solo consulta</span></td></tr>';});
  ...
  function normalizarCoordinador(c){c=c||{};var carreras=listaCarreras(c.carreras||c.carrerasAsignadas);return{id:texto(c.id||c._docId||c.idRegistro||c.coordinadorId||c.nombre),nombre:texto(c.nombre||c.Nombre||c.coordinador),telegram:texto(c.telegram||c.Telegram),activo:c.activo!==false&&texto(c.estado||'ACTIVO').toUpperCase()!=='INACTIVO',carreras:carreras};}
  ...
  function normalizarTitulo(t){t=t||{};return{cedula:cedula(t.cedula||t.numeroIdentificacion),nombres:texto(t.estudiante||t.nombres||t.Nombres),carrera:texto(t.carrera||t.NombreCarrera||t.nombreCarrera),periodo:texto(t.periodo||t.periodoLabel||t.periodoId),estado:texto(t.estado||t.estadoFinal||'PENDIENTE_REVISION').toUpperCase(),fecha:texto(t.fechaEnvio||t['Fecha envío']||t.fechaServidor),titulo1:texto(t.titulo1||t['Título 1']),titulo2:texto(t.titulo2||t['Título 2']),titulo3:texto(t.titulo3||t['Título 3']),preferido:texto(t.preferido||t.tituloPreferido),raw:t};}
  ...
  function diagnosticar(){
    busy(true,'Comprobando servicios...');
    return Promise.allSettled([api().configTitulos(),api().configRequisitos(),api().pingTitulos(),api().pingRequisitos(),api().listarIA()]).then(function(partes){
      var nombres=['Configuración Títulos','Configuración Requisitos','PING Títulos','PING Requisitos','Claves / IA'];
      ...
      badge('ad-badge-requisitos','Requisitos: '+(requisitosOk?'activo':'error'),requisitosOk?'success':'danger');
      badge('ad-badge-claves','Claves: '+(clavesOk?'activo':'error'),clavesOk?'success':'danger');
    }).finally(function(){busy(false);});
  }

  function refrescarTodo(){
    busy(true,'Cargando Google Sheets...');
    return Promise.allSettled([cargarPeriodos(),cargarCoordinadores(),cargarTitulos(),cargarIA()]).then(function(partes){
      ...
      estadoBox('ad-estado-general',errores.length?'Carga parcial:\n'+errores.join('\n'):'Datos cargados correctamente desde Google Sheets.',errores.length?'warning':'success');
      return diagnosticar();
    }).finally(function(){busy(false);});
  }
