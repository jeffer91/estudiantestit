...
(function(window,document){
  ...
  if(window.AD_CONFIG){
    ...
    window.AD_CONFIG.accionesLog.baseAnalizada=window.AD_CONFIG.accionesLog.baseAnalizada||"ADMIN_BASE_ANALIZADA";
    window.AD_CONFIG.accionesLog.tituloNormalizado=window.AD_CONFIG.accionesLog.tituloNormalizado||"ADMIN_TITULO_NORMALIZADO";
    window.AD_CONFIG.accionesLog.duplicadoDetectado=window.AD_CONFIG.accionesLog.duplicadoDetectado||"ADMIN_DUPLICADO_DETECTADO";
    window.AD_CONFIG.accionesLog.titulosDevueltos=window.AD_CONFIG.accionesLog.titulosDevueltos||"ADMIN_TITULOS_DEVUELTOS";
  ...
  function agregarCss(){
    ...
    agregarCssArchivo("ad-sheets-css","./ad-css/ad-sheets.css");
  ...
  function htmlEstudiantes(){
    return[
      ...
      '<label><span>Período</span><select id="ad-estudiantes-periodo"><option value="">Cargando períodos...</option></select></label>',
      '<label><span>Carrera</span><select id="ad-estudiantes-carrera"><option value="">Todas las carreras</option></select></label>',
      '<label><span>Estado</span><select id="ad-estudiantes-estado"><option value="">Todos los estados</option><option value="NO_ENVIO">No envió</option><option value="ENVIADO">Envió</option><option value="DEVUELTO">Devuelto</option><option value="APROBADO">Aprobado</option></select></label>',
      '<label class="ad-estudiantes-buscador"><span>Buscar estudiante</span><input id="ad-estudiantes-busqueda" type="search" placeholder="Cédula, nombre o carrera" autocomplete="off"></label>',
      '</div>',
      '<div class="ad-estudiantes-resumen" aria-live="polite"><span>Total <strong id="ad-estudiantes-total">0</strong></span><span>Enviaron <strong id="ad-estudiantes-enviaron">0</strong></span><span>No enviaron <strong id="ad-estudiantes-no-enviaron">0</strong></span><span>Devueltos <strong id="ad-estudiantes-devueltos">0</strong></span></div>',
      '<div id="ad-estado-estudiantes" class="ad-status-box">Selecciona un período para cargar estudiantes.</div>',
      '<div class="ad-table-wrap"><table class="ad-table ad-tabla-estudiantes"><thead><tr><th>Cédula</th><th>Nombre</th><th>Carrera</th><th>Estado</th><th>WhatsApp</th><th>Ver más</th></tr></thead><tbody id="ad-tabla-estudiantes"><tr><td colspan="6" class="ad-empty">Sin estudiantes cargados.</td></tr></tbody></table></div>',
      '</div>'
    ].join("");
  }
  ...
  function agregarModal(){
    ...
    modal.innerHTML='<div class="ad-modal-backdrop" data-ad-modal-cerrar></div><div class="ad-modal-card"><header class="ad-modal-header"><div><p class="ad-eyebrow">Detalle del estudiante</p><h3 id="ad-estudiante-modal-titulo">Información</h3><p id="ad-estudiante-modal-subtitulo" class="ad-muted"></p></div><button class="ad-icon-btn" type="button" data-ad-modal-cerrar aria-label="Cerrar">×</button></header><div class="ad-modal-body" id="ad-estudiante-modal-contenido"></div><footer class="ad-modal-footer"><button class="ad-btn ad-btn-secondary" type="button" data-ad-modal-cerrar>Cerrar</button></footer></div>';
    document.body.appendChild(modal);
  }

  function agregarSeccionDinamica(id,label,referenciaId){
    var nav=document.querySelector(".ad-nav");
    var referenciaNav=document.querySelector('[data-ad-view-target="'+referenciaId+'"]');
    var enlace;
    var referenciaSeccion=$(referenciaId)||$("ad-seccion-diagnostico")||$("ad-seccion-logs");
    var seccion;

    if(nav&&!document.querySelector('[data-ad-view-target="'+id+'"]')){
