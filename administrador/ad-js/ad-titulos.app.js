...
(function(window,document){
  ...
  function htmlEstudiantes(){
    return [
      ...
      '    <label>',
      '      <span>Período</span>',
      '      <select id="ad-estudiantes-periodo">',
      '        <option value="">Cargando períodos...</option>',
      '      </select>',
      '    </label>',
      '    <label>',
      '      <span>Carrera</span>',
      '      <select id="ad-estudiantes-carrera">',
      ...
      '        </tr>',
      '      </thead>',
      '      <tbody id="ad-tabla-estudiantes">',
      '        <tr><td colspan="6" class="ad-empty">Sin estudiantes cargados.</td></tr>',
      '      </tbody>',
      '    </table>',
      '  </div>',
      '</div>'
  ...
  function agregarModal(){
    ...
    modal.innerHTML = [
      ...
      '    <button class="ad-btn ad-btn-secondary" type="button" data-ad-modal-cerrar>Cerrar</button>',
      '  </footer>',
      '</div>'
    ].join("");
  ...
  function instalarNavegacion(){
    document.addEventListener("click",function(evento){
      var enlace = evento.target && evento.target.closest
        ? evento.target.closest("[data-ad-view-target]")
        : null;
