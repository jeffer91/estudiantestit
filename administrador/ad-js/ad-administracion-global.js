...
(function(window,document){
  ...
  var VERSION='3.6.0';
  var state={periodos:[],carreras:[],coordinadores:[],investigadores:[],investigacion:{ultimoPorEnvio:{},bloqueos:[],revisiones:[]},global:null,estadisticas:null,detalle:null};
  ...
  function statusLabel(value){var status=texto(value).toUpperCase();if(status==='NO_ENVIADO')return'No enviado';if(status==='PENDIENTE_INVESTIGADOR')return'Pendiente de Investigación';if(status==='APROBADO_FINAL')return'Aprobado final';if(status==='APROBADO')return'Aprobado histórico';if(status==='REEMPLAZADO')return'Aprobado con corrección histórico';if(status==='DEVUELTO')return'Devuelto';return'Pendiente de Coordinación';}
  function statusClass(value){var status=texto(value).toUpperCase();if(status==='NO_ENVIADO')return'ad-badge-warning';if(status==='APROBADO_FINAL'||status==='APROBADO'||status==='REEMPLAZADO')return'ad-badge-success';if(status==='DEVUELTO')return'ad-badge-warning';return'ad-badge-info';}
  ...
  function styles(){
    ...
    style.textContent=''+
      ...
      '.ad-v2-toolbar{display:grid;grid-template-columns:1fr 1.15fr 1fr 1.1fr 1.1fr 1.3fr;gap:12px;align-items:end;margin-bottom:15px}'+
  ...
  function createModals(){
    ...
    wrapper.innerHTML=''+
      '<section class="ad-v2-modal" id="ad-v2-detail-modal" hidden><div class="ad-v2-modal__backdrop" data-v2-action="close-detail"></div><div class="ad-v2-modal__card"><div class="ad-v2-modal__head"><div><p class="ad-eyebrow">Detalle del registro</p><h3 id="ad-v2-detail-name">Estudiante</h3></div><button class="ad-v2-icon" type="button" data-v2-action="close-detail">✕</button></div><div class="ad-v2-detail"><div><span>Cédula</span><strong id="ad-v2-detail-id">-</strong></div><div><span>Carrera</span><strong id="ad-v2-detail-career">-</strong></div><div><span>Período</span><strong id="ad-v2-detail-period">-</strong></div><div><span>Fecha de envío</span><strong id="ad-v2-detail-date">-</strong></div></div><div class="ad-v2-proposals" id="ad-v2-proposals"></div><div class="ad-v2-review"><h4>Revisión de Coordinación</h4><div class="ad-v2-review-grid"><div><span>Estado</span><strong id="ad-v2-detail-state">-</strong></div><div><span>Coordinador</span><strong id="ad-v2-detail-coordinator">-</strong></div><div><span>Título validado</span><strong id="ad-v2-detail-final">-</strong></div><div><span>Fecha</span><strong id="ad-v2-detail-resolution-date">-</strong></div><div><span>Observación</span><strong id="ad-v2-detail-observation">-</strong></div></div></div><div class="ad-v2-review"><h4>Revisión de Investigación</h4><div class="ad-v2-review-grid"><div><span>Investigador</span><strong id="ad-v2-investigator-name">-</strong></div><div><span>Resultado</span><strong id="ad-v2-investigator-result">-</strong></div><div><span>Título antes</span><strong id="ad-v2-investigator-before">-</strong></div><div><span>Título después</span><strong id="ad-v2-investigator-after">-</strong></div><div><span>Fecha</span><strong id="ad-v2-investigator-date">-</strong></div><div><span>Observación</span><strong id="ad-v2-investigator-observation">-</strong></div></div></div><div class="ad-v2-review"><h4>Historial del proceso</h4><div id="ad-v2-history">Cargando historial...</div></div><div class="ad-v2-return"><label><strong>Motivo para devolver</strong><textarea id="ad-v2-return-reason" placeholder="Escribe una observación de al menos 4 caracteres"></textarea></label><pre id="ad-v2-detail-status" class="ad-result-box">Puedes devolver o eliminar el envío.</pre></div><div class="ad-v2-modal__foot"><button class="ad-btn ad-btn-secondary" type="button" data-v2-action="close-detail">Cerrar</button><button class="ad-btn ad-btn-danger" type="button" data-v2-action="delete-detail">🗑️ Eliminar</button><button class="ad-btn ad-btn-primary" type="button" data-v2-action="return-detail">↩️ Devolver</button></div></div></section>'+
  ...
  function investigationOf(item){var id=texto(item&&item.envioId);return id&&state.investigacion&&state.investigacion.ultimoPorEnvio?state.investigacion.ultimoPorEnvio[id]||null:null;}
  function fillResponsibleFilters(){var co=$('ad-v2-title-coordinator'),inv=$('ad-v2-title-investigator');if(co){var prev=texto(co.value);co.innerHTML='<option value="">Todos</option>'+state.coordinadores.map(function(x){var n=texto(x.nombre||x.coordinador||x.id);return option(n,n,false);}).join('');if(prev)co.value=prev;}if(inv){var prevI=texto(inv.value);inv.innerHTML='<option value="">Todos</option>'+state.investigadores.map(function(x){var n=texto(x.nombre||x.cedula);return option(n,n,false);}).join('');if(prevI)inv.value=prevI;}}
  function fillCareerFilters(){
    var names=[];(state.global&&state.global.registros||[]).forEach(function(item){if(item.carrera&&names.indexOf(item.carrera)<0)names.push(item.carrera);});if(!names.length)state.carreras.forEach(function(item){if(item.nombre&&names.indexOf(item.nombre)<0)names.push(item.nombre);});names.sort(function(a,b){return a.localeCompare(b,'es');});['ad-v2-title-career','ad-v2-stat-career'].forEach(function(id){var select=$(id);if(!select)return;var previous=texto(select.value);select.innerHTML='<option value="">Todas</option>'+names.map(function(name){return option(name,name,false);}).join('');if(previous&&names.indexOf(previous)>=0)select.value=previous;});fillResponsibleFilters();
  }

  function loadCareers(){
    setStatus('ad-v2-career-status','Cargando carreras y coordinadores...','info');
    return Promise.all([api().listarCarrerasAdmin(),api().listarCoordinadores(),api().listarInvestigadores?api().listarInvestigadores():Promise.resolve({investigadores:[]})]).then(function(results){state.carreras=results[0].carreras||results[0].registros||[];state.coordinadores=results[1].coordinadores||results[1].registros||[];state.investigadores=results[2].investigadores||results[2].registros||[];renderCareers();setStatus('ad-v2-career-status','Selecciona un coordinador para guardar automáticamente la asignación.','success');}).catch(function(error){setStatus('ad-v2-career-status',message(error),'danger');throw error;});
  }
  ...
  function loadGlobal(){
    ...
    return Promise.all([api().listarTitulosGlobal({periodoId:period,periodo:period}),api().resumenInvestigacion?api().resumenInvestigacion().catch(function(){return{ultimoPorEnvio:{},bloqueos:[],revisiones:[]};}):Promise.resolve({ultimoPorEnvio:{},bloqueos:[],revisiones:[]})]).then(function(results){var result=results[0];state.investigacion=results[1]||{ultimoPorEnvio:{},bloqueos:[],revisiones:[]};window.ADAdminInvestigationLast=state.investigacion;state.global=result;window.ADAdminGlobalLast=result;renderGlobal();setStatus('ad-v2-title-status',result.mensaje||'Lista global cargada.','success');return result;}).catch(function(error){setStatus('ad-v2-title-status',message(error),'danger');throw error;}).finally(function(){busy(false);});
  }

  function filteredGlobal(){
    var rows=state.global&&state.global.registros||[];var career=texto($('ad-v2-title-career')&&$('ad-v2-title-career').value);var status=texto($('ad-v2-title-state')&&$('ad-v2-title-state').value);var coordinator=texto($('ad-v2-title-coordinator')&&$('ad-v2-title-coordinator').value);var investigator=texto($('ad-v2-title-investigator')&&$('ad-v2-title-investigator').value);var search=normal($('ad-v2-title-search')&&$('ad-v2-title-search').value);return rows.filter(function(item){var rev=investigationOf(item);if(career&&normal(item.carrera)!==normal(career))return false;if(status&&item.estado!==status)return false;if(coordinator&&normal(item.coordinador)!==normal(coordinator))return false;if(investigator&&normal(rev&&rev.investigadorNombre)!==normal(investigator))return false;if(search&&normal([item.cedula,item.nombres,item.carrera,item.estado,item.titulo1,item.titulo2,item.titulo3,item.tituloCoordinador,item.tituloFinal,item.coordinador,rev&&rev.investigadorNombre].join(' ')).indexOf(search)<0)return false;return true;});
  }
  ...
  function openDetail(student){
    if(!student||student.estado==='NO_ENVIADO')return;state.detalle=student;var inv=investigationOf(student)||{};
    $('ad-v2-detail-name').textContent=student.nombres||'Estudiante';$('ad-v2-detail-id').textContent=student.cedula||'-';$('ad-v2-detail-career').textContent=student.carrera||'-';$('ad-v2-detail-period').textContent=periodLabel(selectedPeriod('ad-v2-title-period'));$('ad-v2-detail-date').textContent=formatDate(student.fechaEnvio);$('ad-v2-detail-state').textContent=statusLabel(student.estado);$('ad-v2-detail-coordinator').textContent=student.coordinador||'Sin revisión';$('ad-v2-detail-final').textContent=student.tituloCoordinador||student.tituloFinal||'-';$('ad-v2-detail-resolution-date').textContent=formatDate(student.fechaValidacionCoordinador||student.fechaResolucion);$('ad-v2-detail-observation').textContent=student.observacion||'-';$('ad-v2-investigator-name').textContent=inv.investigadorNombre||'-';$('ad-v2-investigator-result').textContent=inv.resultado||student.resultadoInvestigacion||'-';$('ad-v2-investigator-before').textContent=inv.tituloAntes||student.tituloCoordinador||'-';$('ad-v2-investigator-after').textContent=inv.tituloDespues||student.tituloFinal||'-';$('ad-v2-investigator-date').textContent=formatDate(inv.fecha||student.fechaResolucionInvestigacion);$('ad-v2-investigator-observation').textContent=inv.observacion||student.observacionInvestigacion||'-';$('ad-v2-history').textContent='Cargando historial...';$('ad-v2-return-reason').value='';setStatus('ad-v2-detail-status','Puedes devolver o eliminar el envío.','info');
    $('ad-v2-proposals').innerHTML=[1,2,3].map(function(number){var title=student['titulo'+number]||'-';var favorite=Number(student.tituloPreferidoNumero)===number;return '<article class="ad-v2-proposal '+(favorite?'is-favorite':'')+'"><strong>Título '+number+(favorite?' · ⭐ Favorito':'')+'</strong><p>'+esc(title)+'</p></article>';}).join('');openModal('ad-v2-detail-modal');if(api().consultarHistorialTitulo){api().consultarHistorialTitulo({envioId:student.envioId,cedula:student.cedula,periodoId:selectedPeriod('ad-v2-title-period'),periodo:periodLabel(selectedPeriod('ad-v2-title-period'))}).then(function(h){var eventos=h.lineaTiempo||h.eventosWorkflow||[];$('ad-v2-history').innerHTML=eventos.length?eventos.map(function(e){var cambio=e.tituloAntes&&e.tituloDespues&&normal(e.tituloAntes)!==normal(e.tituloDespues)?'<br><small>Antes: '+esc(e.tituloAntes)+'</small><br><small>Después: '+esc(e.tituloDespues)+'</small>':'';return '<div style="padding:8px 0;border-bottom:1px solid #dde6f0"><strong>'+esc([e.rol,e.accion,e.resultado].filter(Boolean).join(' · '))+'</strong><br><small>'+esc(formatDate(e.fecha))+'</small>'+cambio+(e.observacion?'<p>'+esc(e.observacion)+'</p>':'')+'</div>';}).join(''):'Sin eventos adicionales.';}).catch(function(error){$('ad-v2-history').textContent='Historial no disponible: '+message(error);});}
  }
  ...
  function events(){
    ...
    document.addEventListener('change',function(event){var careerId=event.target.getAttribute('data-v2-career-select');if(careerId!==null){assignCareer(careerId,texto(event.target.value));return;}if(event.target.id==='ad-v2-title-period')loadGlobal();else if(['ad-v2-title-career','ad-v2-title-state','ad-v2-title-coordinator','ad-v2-title-investigator'].indexOf(event.target.id)>=0)renderGlobal();else if(event.target.id==='ad-v2-stat-period')loadStats();});
