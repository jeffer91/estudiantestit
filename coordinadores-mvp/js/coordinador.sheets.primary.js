/* Coordinadores: toda la operación se realiza con RESPALDO TITULOS APP. */
(function(window){
'use strict';
...
function mensajeError(v){return v&&v.message?v.message:typeof v==='string'?v:'Error de Google Sheets.';}
...
function solicitar(a,p,m){return fetch(url(),{method:'POST',cache:'no-store',headers:{'Content-Type':'application/json','X-Titulos-App':'coordinadores'},body:JSON.stringify({accion:a,metodo:m||'POST',datos:p||{}})}).then(function(resp){return resp.text().then(function(body){var j={};try{j=body?JSON.parse(body):{};}catch(e){throw new Error('RESPALDO TITULOS APP respondió en formato no válido.');}if(!resp.ok||j.ok===false)throw new Error(j.mensaje||j.error||('Error HTTP '+resp.status));return j;});});}
function tget(a,p){return solicitar(a,p,'GET');}
function tpost(a,p){return solicitar(a,p,'POST');}

function leerConfiguracion(forzar){return resolverUnaVez('configuracion',CACHE_CONFIG_MS,forzar===true,function(){return solicitar('CONFIGURACION_PUBLICA',{},'GET').then(function(x){return{activo:x.activo!==false,titulos:x,origen:'RESPALDO TITULOS APP'};});});}
...
function diagnostico(){return Promise.all([leerConfiguracion(),tget('PING',{})]).then(function(p){return{ok:true,fuentePrincipal:'RESPALDO TITULOS APP',titulos:p[1],configuracion:p[0],fecha:new Date().toISOString()};});}
