/* Coordinadores: lectura y escritura exclusiva en Firebase Títulos. */
(function(window){
'use strict';
...
function normalizarEnvio(f,i){
  ...
  return{
    id:id||[f.periodoId||p,f.cedula||f.numeroIdentificacion||'',i].join('__'),_clave:id||[f.periodoId||p,f.cedula||f.numeroIdentificacion||'',i].join('__'),
    ...
    coordinador:utils().limpiarTexto(campo(f,c.coordinador,'')||f.coordinador),fechaRevision:utils().limpiarTexto(campo(f,c.fechaRevision,'')||f.fechaResolucion),fuente:'FIREBASE_TITULOS',raw:f
  };
}
...
function listarEnvios(){return tget('LISTAR_ENVIOS_POR_CARRERA',{estado:'',todas:'true',incluirTodos:'true'}).then(function(r){return lista(r,'envios').map(normalizarEnvio).filter(function(x){return x&&x.cedula&&x.tieneTitulos;});});}
