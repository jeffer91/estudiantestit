...
- Conservar los mensajes de error al crear snapshots del estado.
========================================================= */
(function(window){
  'use strict';
  ...
  function normal(valor){
    return texto(valor).toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^A-Z0-9]+/g,' ').replace(/\s+/g,' ').trim();
  }
  ...
  function errorSerializable(error){
    ...
    if(typeof error==='object'){
      ...
      if(typeof mensaje==='object'){
        try{mensaje=JSON.stringify(mensaje);}catch(errorJson){mensaje=String(mensaje);}
      }
      if(!mensaje){
        try{mensaje=JSON.stringify(error);}catch(errorJson2){mensaje=String(error);}
      }
  ...
  function clonar(valor){
    ...
    try{
      return JSON.parse(JSON.stringify(valor,function(clave,item){
        return item instanceof Error?errorSerializable(item):item;
      }));
    }catch(error){
      if(utils()&&utils().clonar){
        try{return utils().clonar(valor);}catch(errorUtils){}
      }
      return valor;
    }
  }
  ...
  function coincideCarrera(envio,coordinador){
    ...
    return carreras.some(function(item){var token=normal(item);if(!token)return false;return valoresEnvio.some(function(valor){return valor===token||valor.indexOf(token)>=0||token.indexOf(valor)>=0;});});
  }
  function coincidePeriodo(envio,periodo){
    if(!periodo)return false;var id=normal(periodo.id);var label=normal(periodo.label);
    return Boolean((id&&normal(envio.periodoId)===id)||(label&&normal(envio.periodoLabel)===label)||(label&&normal(envio.periodo)===label));
  }
  function recalcularFiltros(){
    var permitidos=estadosVista(state.vistaActual);var busqueda=normal(state.busqueda);var periodo=state.periodoActual;var coordinador=state.coordinadorActual;
    state.registrosFiltrados=state.envios.filter(function(envio){
      var textoBusqueda=normal([envio.cedula,envio.nombres,envio.carrera,envio.codigoCarrera,envio.periodoLabel].join(' '));
      return Boolean(coincidePeriodo(envio,periodo)&&coincideCarrera(envio,coordinador)&&permitidos.indexOf(estadoNormal(envio.estado))>=0&&(!busqueda||textoBusqueda.indexOf(busqueda)>=0));
    });
  ...
  function seleccionarEstudiante(id){
    id=texto(id);state.estudianteSeleccionado=state.envios.find(function(item){return item.id===id||item._docId===id||item._clave===id||item.cedula===id;})||null;emitir('estudiante');return obtenerEstudianteSeleccionado();
  }
  ...
  function actualizarEnvioLocal(id,cambios){
    id=texto(id);var actualizado=null;state.envios=state.envios.map(function(item){if(item.id!==id&&item._docId!==id&&item._clave!==id&&item.cedula!==id)return item;actualizado=Object.assign({},item,cambios||{});return actualizado;});
    if(actualizado)state.estudianteSeleccionado=actualizado;recalcularFiltros();emitir('envio-actualizado');return actualizado?clonar(actualizado):null;
  }
  ...
  window.CoordinadorMVPState=Object.freeze({
    ...
    recalcularFiltros:recalcularFiltros,obtenerRegistrosFiltrados:obtenerRegistrosFiltrados,obtenerTotalFiltrado:obtenerTotalFiltrado,
