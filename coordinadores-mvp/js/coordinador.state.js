...
Función:
- Mantener períodos, coordinadores y envíos de Firebase Títulos.
- Separar pendientes de revisión, aprobados y devueltos.
...
(function(window){
  ...
  function setPeriodos(lista,principal){
    var ultimoId=leerLocal('coordinadores_mvp__ultimo_periodo');var seleccionado=null;
  ...
  function setPeriodoActual(id){id=texto(id);state.periodoActual=state.periodos.find(function(item){return item.id===id;})||null;if(state.periodoActual)guardarLocal('coordinadores_mvp__ultimo_periodo',state.periodoActual.id);state.estudianteSeleccionado=null;recalcularFiltros();emitir('periodo');return obtenerPeriodoActual();}
  ...
  function firmaPeriodo(valor){
    ...
    var pares=[],visto={},m;function agregar(anio,mes){mes=String(Number(mes)).padStart(2,'0');var par=anio+'-'+mes;if(Number(mes)>=1&&Number(mes)<=12&&!visto[par]){visto[par]=true;pares.push(par);}}
    ...
    if(pares.length>=2)return pares[0]+'__'+pares[pares.length-1];if(pares.length===1)return pares[0];return base;
  }
  function periodoEquivalente(a,b){var na=normal(a),nb=normal(b);if(!na||!nb)return false;if(na===nb||na.indexOf(nb)>=0||nb.indexOf(na)>=0)return true;var fa=firmaPeriodo(a),fb=firmaPeriodo(b);return Boolean(fa&&fb&&fa===fb);}
  function coincidePeriodo(envio,periodo){if(!periodo)return false;var esperados=[periodo.id,periodo.label].filter(Boolean);var disponibles=[envio&&envio.periodoId,envio&&envio.periodoLabel,envio&&envio.periodo].filter(Boolean);return esperados.some(function(a){return disponibles.some(function(b){return periodoEquivalente(a,b);});});}
  ...
  function recalcularFiltros(){
    var permitidos=estadosVista(state.vistaActual),busqueda=normal(state.busqueda),periodo=state.periodoActual,coordinador=state.coordinadorActual;
    ...
    state.registrosFiltrados=delEstado.filter(function(envio){var base=normal([envio.cedula,envio.nombres,envio.carrera,envio.codigoCarrera,envio.periodoLabel].join(' '));return !busqueda||base.indexOf(busqueda)>=0;});
    state.diagnosticoFiltros={recibidos:state.envios.length,conTitulos:conTitulos.length,delPeriodo:delPeriodo.length,deCarreras:deCarreras.length,delEstado:delEstado.length,mostrados:state.registrosFiltrados.length};
  ...
  window.CoordinadorMVPState=Object.freeze({iniciar:iniciar,obtenerEstado:obtenerEstado,estaCargando:estaCargando,setCargando:setCargando,setError:setError,limpiarError:limpiarError,setPeriodos:setPeriodos,setPeriodoActual:setPeriodoActual,obtenerPeriodos:obtenerPeriodos,obtenerPeriodoActual:obtenerPeriodoActual,setCoordinadores:setCoordinadores,setCoordinadorActual:setCoordinadorActual,obtenerCoordinadores:obtenerCoordinadores,obtenerCoordinadorActual:obtenerCoordinadorActual,setEnvios:setEnvios,obtenerEnvios:obtenerEnvios,setVistaActual:setVistaActual,obtenerVistaActual:obtenerVistaActual,setBusqueda:setBusqueda,obtenerBusqueda:obtenerBusqueda,recalcularFiltros:recalcularFiltros,obtenerRegistrosFiltrados:obtenerRegistrosFiltrados,obtenerTotalFiltrado:obtenerTotalFiltrado,obtenerDiagnosticoFiltros:obtenerDiagnosticoFiltros,seleccionarEstudiante:seleccionarEstudiante,setEstudianteSeleccionado:setEstudianteSeleccionado,obtenerEstudianteSeleccionado:obtenerEstudianteSeleccionado,actualizarEnvioLocal:actualizarEnvioLocal,limpiar:limpiar,escuchar:escuchar});
})(window);
