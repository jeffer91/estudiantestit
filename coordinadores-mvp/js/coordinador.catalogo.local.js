...
(function(window){
  ...
  function guardarCatalogo(lista){if(!Array.isArray(lista)||!lista.length)return false;try{window.localStorage.setItem(STORAGE_KEY,JSON.stringify({actualizadoEn:new Date().toISOString(),origen:'google-sheets',total:lista.length,coordinadores:lista}));return true;}catch(error){return false;}}
  ...
  function crearServicioConFallback(servicioOriginal){
    ...
    servicio.listarCoordinadores=function(forzar){return Promise.resolve().then(function(){return listarOriginal.call(servicioOriginal,forzar===true);}).then(function(lista){if(Array.isArray(lista)&&lista.length){guardarCatalogo(lista);return lista;}throw new Error('La hoja Coordinadores no devolvió registros activos.');}).catch(function(errorSheets){var listaLocal=normalizarListaLocal(leerCatalogo(),normalizar);if(listaLocal.length)return listaLocal;throw new Error('La hoja Coordinadores no respondió y todavía no existe un catálogo local en este navegador. Error: '+(errorSheets&&errorSheets.message?errorSheets.message:String(errorSheets)));});};
