...
(function(window,document){
  ...
  function render(snapshot){
    ...
    pintarResumen(snapshot.coordinadorActual);pintarTabs(snapshot.vistaActual);pintarEncabezado(snapshot.vistaActual);
    pintarTabla(snapshot.registrosFiltrados||[]);setTexto('contadorRegistros',String((snapshot.registrosFiltrados||[]).length));
    actualizarEstado(snapshot);setCargando(snapshot.cargando,'Cargando...');
  }
  ...
  function pintarCoordinadores(lista,actual,periodo){
    var select=$('coordinadorSelect');var valor=actual&&actual.id?actual.id:(select&&select.value);if(!select)return;
    select.disabled=!periodo||!lista.length;
    if(!lista.length){select.innerHTML='<option value="">No hay coordinadores activos</option>';select.value='';return;}
    select.innerHTML='<option value="">Selecciona un coordinador</option>'+lista.map(function(item){return'<option value="'+esc(item.id)+'">'+esc(item.nombre||item.id)+'</option>';}).join('');
    if(valor&&lista.some(function(item){return item.id===valor;}))select.value=valor;
  }
  function pintarResumen(coordinador){
    var resumen=$('coordinadorResumen');if(!resumen)return;
    if(!coordinador){resumen.hidden=true;return;}
  ...
  function setCargando(activo,mensaje){var overlay=$('loadingOverlay');var label=$('loadingTexto');if(overlay)overlay.hidden=!activo;if(label&&mensaje)label.textContent=mensaje;}
