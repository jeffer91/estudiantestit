/* Coordinadores: operación exclusiva sobre Firebase Títulos. */
...
function resumen(){var diagnostico=state().obtenerDiagnosticoFiltros?state().obtenerDiagnosticoFiltros():{};estado(diagnostico.mostrados?'Mostrando '+diagnostico.mostrados+' título(s), sin filtro de período.':'No hay coincidencias para el coordinador y el estado seleccionados.',diagnostico.mostrados?'success':'warning');}

function cargarCatalogos(forzar){
  state().limpiarError();state().setCargando(true);
  estado(forzar?'Actualizando Firebase Títulos...':'Cargando coordinadores y todos los envíos de Firebase Títulos...','info');
  ...
  return Promise.all([sheets().listarEnvios({forzar:forzar===true}),sheets().listarCoordinadores(forzar===true)]).then(function(resultados){
    var envios=Array.isArray(resultados[0])?resultados[0]:[];
    var coordinadores=Array.isArray(resultados[1])?resultados[1]:[];
    state().setCoordinadores(coordinadores);state().setEnvios(envios);state().limpiarError();
    if(state().obtenerCoordinadorActual())resumen();else estado('Firebase Títulos cargado. Selecciona un coordinador.','success');
    return{ok:true,coordinadores:coordinadores,envios:envios};
...
function cargarTitulos(forzar){state().setCargando(true);return sheets().listarEnvios({forzar:forzar===true}).then(function(lista){state().setEnvios(lista||[]);resumen();return lista||[];}).catch(function(error){estado('No se pudieron actualizar los envíos: '+textoError(error),'error');return[];}).finally(function(){state().setCargando(false);});}
function abrirDetalle(id){var envio=state().seleccionarEstudiante(id);if(!envio){estado('No se encontró el envío.','error');return;}state().setEstudianteSeleccionado(envio);modal().abrir(envio);}
