...
'use strict';
var iniciado=false,fuenteActual='RESPALDO TITULOS APP';
...
function textoError(e){if(!e)return'Error desconocido.';if(typeof e==='string')return e;if(e.message)return textoError(e.message);if(e.mensaje)return textoError(e.mensaje);if(e.error)return textoError(e.error);try{return JSON.stringify(e);}catch(x){return String(e);}}
function validar(){var f=[];if(!state())f.push('CoordinadorMVPState');if(!ui())f.push('CoordinadorMVPUI');if(!modal())f.push('CoordinadorMVPModal');if(!sheets())f.push('CoordinadorMVPSheetsPrimary');if(f.length)throw new Error('Faltan módulos: '+f.join(', '));}
function estado(m,t){ui().mostrarEstado('estadoPrincipal',m,t||'info');}
function resumen(){var d=state().obtenerDiagnosticoFiltros?state().obtenerDiagnosticoFiltros():{};estado(d.mostrados?'Mostrando '+d.mostrados+' estudiante(s).':'No hay coincidencias para los filtros seleccionados.',d.mostrados?'success':'warning');}

function cargarCatalogos(forzar){
 state().limpiarError();state().setCargando(true);estado(forzar?'Actualizando RESPALDO TITULOS APP...':'Cargando períodos, coordinadores y envíos...','info');
 if(forzar&&sheets().invalidarCacheEnvios)sheets().invalidarCacheEnvios();
 if(forzar&&sheets().limpiarCache)sheets().limpiarCache();
 return Promise.all([sheets().listarPeriodos({forzar:forzar===true}),sheets().listarCoordinadores(forzar===true)])
 .then(function(p){var periodos=p[0]||{},coordinadores=Array.isArray(p[1])?p[1]:[];if(!periodos.periodos||!periodos.periodos.length)throw new Error('RESPALDO TITULOS APP no devolvió períodos disponibles.');state().setPeriodos(periodos.periodos,periodos.principal||periodos.periodos[0]);state().setCoordinadores(coordinadores);state().setEnvios(Array.isArray(periodos.envios)?periodos.envios:[]);fuenteActual='RESPALDO TITULOS APP';state().limpiarError();if(state().obtenerCoordinadorActual())resumen();else estado('Datos cargados. Selecciona un coordinador.','success');return{ok:true,periodos:periodos.periodos,coordinadores:coordinadores,envios:periodos.envios||[]};})
 .catch(function(e){var m=textoError(e);state().setPeriodos([],null);state().setCoordinadores([]);state().setEnvios([]);state().setError(new Error(m));estado(m,'error');return{ok:false,error:m};})
