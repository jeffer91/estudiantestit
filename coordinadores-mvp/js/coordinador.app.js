/* Coordinadores: operación sobre artículos y Trabajos de Titulación. */
...
function cargarCatalogos(forzar){
  ...
  return Promise.all([sheets().listarEnvios({forzar:forzar===true}),sheets().listarCoordinadores(forzar===true)]).then(function(resultados){
    ...
    if(state().obtenerCoordinadorActual())resumen();else estado('Datos cargados. Selecciona un coordinador.','success');
