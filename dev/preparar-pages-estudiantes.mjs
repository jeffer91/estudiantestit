...
function injectScript(html, sourcePath) {
  ...
  if (!html.includes('</body>')) throw new Error('No se encontró </body> para insertar un script requerido.');
...
workHtml = injectScript(workHtml, WORK_PREFILL_FILE);
...
const required = [
  ...
  path.join(publicWork, WORK_PREFILL_FILE)
];
