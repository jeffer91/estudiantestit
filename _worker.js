...
function safeEndpoint(value) {
  ...
  if (!raw) {
    throw providerError("El proveedor no tiene endpoint configurado.", 0, "ENDPOINT_MISSING");
  }
  ...
  if (!allowed) {
    throw providerError(
      "El dominio del endpoint no está permitido por el proxy IA.",
      0,
...
function geminiEndpoint(provider, apiKey) {
  ...
  const model = text(provider.modelo || provider.model || "gemini-2.5-flash");
  if (!apiKey) {
    throw providerError("Gemini no tiene API key configurada.", 0, "API_KEY_MISSING");
  }
...
function openAiHeaders(provider, apiKey) {
  if (!apiKey) {
    throw providerError(
      "El proveedor no tiene API key o token configurado.",
      0,
      "API_KEY_MISSING"
    );
  }
...
async function fetchWithTimeout(endpoint, options, timeoutMs) {
  ...
  try {
    ...
    if (error && error.name === "AbortError") {
      throw providerError(
        "El proveedor superó el tiempo máximo de espera.",
        0,
        "TIMEOUT"
      );
    }
...
async function callProvider(provider, prompt, options) {
  ...
  if (type === "gemini") {
    ...
    if (!model) {
      throw providerError("El proveedor no tiene modelo configurado.", 0, "MODEL_MISSING");
    }
...
async function handlePost(request) {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    return json(request, {
      ok: false,
      error: "Se esperaba application/json.",
...
async function handleIaRequest(request) {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/ia" || url.pathname === "/api/ia/") {
      return handleIaRequest(request);
    }
